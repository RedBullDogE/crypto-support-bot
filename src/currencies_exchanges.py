...
def check_correct_cmd(cmd, calculated):
    reg_exp = (
        r"[0-9]+ *([A-Za-z]{2,20}->[A-Za-z]{2,20})"
        if calculated
        else r"([A-Za-z]{2,20}-*>*[A-Za-z]{0,20})"
    )
...
async def get_crypto_currency_info(message, calculating: bool = False):
    ...
    current_currency = (
        re.findall(r"^\d+ *(.*)", message.text)[0] if calculating else message.text
    )
    currency_exchange = ""
            ...
            if currency != current_currency:
                try:
                    res = await client.get_symbol_ticker(
                        symbol=f"{current_currency}{currency}"
                    )
                    ...
                    try:
                        res = await client.get_symbol_ticker(
                            symbol=f"{currency}{current_currency}"
                        )
                        currency_exchange += (
                            f"{number_of_coins} {current_currency}="
                        ...
                        currency_exchange += (
                            f"{current_currency} to {currency} not supported\n"
                        )
    else:
        currency, exchange_to = (
            current_currency.split("->")[0].upper(),
            current_currency.split("->")[1].upper(),
        )
...
async def get_fiat_currency_info(message, calculating=False):
    ...
    current_currency = (
        f"B{current_currency}" if current_currency == "USD" else current_currency
    )
    currency_exchange = ""
    if "->" not in current_currency:
        for currency in ["BUSD", "UAH", "EUR", "BTC", "ETH"]:
            if currency != current_currency:
                try:
                    eth_to_current_currency = await client.get_symbol_ticker(
                        symbol=f"ETH{current_currency}"
                    )
                    eth_to_convert_currency = (
                        await client.get_symbol_ticker(symbol=f"ETH{currency}")
                        if currency != "ETH"
                        else {"price": 1}
                    )
                    res = float(eth_to_convert_currency["price"]) / float(
                        eth_to_current_currency["price"]
                    )
                    currency_exchange += (
                        f"{number_of_coins} {current_currency.replace('BUSD', 'USD')}="
                    ...
                    # Binance does not always support information for the current currency
                    currency_exchange += (
                        f"{current_currency.replace('BUSD', 'USD')} to "
        ...
        try:
            ...
            eth_to_convert_currency = (
                await client.get_symbol_ticker(symbol=f"ETH{exchange_to}")
                if currency != "ETH"
                else {"price": 1}
            )
            res = float(eth_to_convert_currency["price"]) / float(
